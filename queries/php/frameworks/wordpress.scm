...
;
; Framework VOCABULARY on the plugin doctrine: the engine carries no
; WordPress names — the `#any-of?` predicates here are the whole framework
; surface, and the minted refs are ordinary FunctionCall/MethodCall refs
; riding the string-named capture vocabulary (`@ref.call.named` /
; `@ref.method.named`, docs/prompt-pack-plugins.md). A framework's names
; belong in a document like this one; core dispatches generically.
;
...
; characters. Hook-NAME identity (`do_action('init')` ↔ its
; registrations) rides the `hook` Handler rail below.
...
; Hook-NAME identity on the `hook` rail: `add_action('init', …)`'s FIRST
; argument declares the hook (a stacked Handler named by the string
; content), and `do_action('init')` / `apply_filters('the_title', …)` fire
; it (a DispatchCall labeled by the firing function via @dispatch.via).
...
; The rail suffix is the namespace: WordPress hooks are one string space,
; not the program's, so a same-spelled name on another rail stays apart.
(function_call_expression
  function: (name) @_wphook_def
  arguments: (arguments
    . (argument (string (string_content) @def.handler.named.hook)))
  (#any-of? @_wphook_def "add_action" "add_filter"))

(function_call_expression
  function: (name) @dispatch.via
  arguments: (arguments
    . (argument (string (string_content) @ref.dispatch.named.hook)))
  (#any-of? @dispatch.via
