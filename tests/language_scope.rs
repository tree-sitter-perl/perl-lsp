...
/// A regression class pinned through the REAL CLI (the unit net
...
/// instance-receiver path never covered this lane (Laravel's
/// `Model::query()` shape).
#[cfg(feature = "php")]
#[test]
...
/// `parent::` through a same-leaf ALIASED parent
...
/// Three same-leaf classes, one `use` row. A name-keyed
...
/// A property and a method sharing a name on one class keep their own
/// identity. The member family rides the ref kind (`$this->recorded` is a
/// `FieldAccess`, `$f->recorded()` a `MethodCall`), the hop (`ValueHop`
/// chases the class's value edge, `MethodHop` its callable) and the walk
/// (`resolve_field_in_ancestors` admits value kinds only,
/// `docs/adr/member-kinds.md`): hover/gd on the access land on the
/// property, refs/rename from either declaration stay on their own side.
#[cfg(feature = "php")]
#[test]
...
/// Static calls on EXPRESSION receivers (`$this->prop::m()`,
...
/// The php tier's heatmap fan-in is pre-pruned from its OWN
...
fn php_heatmap_pre_prune_preserves_every_fan_in() {
    ...
    // `Svc::__construct` has no `new` site, but `Svc` is named by a
    // type hint — a container instantiates it, so the ctor is shielded
    // (`class-referenced`); `Consumer`'s ctor (its class named nowhere)
    // stays a candidate. The shield is the class's own `references()`
    // projection, so it holds with the row store OFF too — it is a guard,
    // not a row-store verdict.
    for rows in ["1", "0"] {
        let full = run_full(rows);
        ...
        assert_eq!(svc["reachable_guard"].as_str(), Some("class-referenced"), "rows={rows}: {svc}");
        assert_eq!(svc["dead_code_candidate"], false, "rows={rows}: {svc}");
...
/// A nested generic whose innermost element is `mixed`
...
/// An aliased import pins the ALIAS spelling, never the
