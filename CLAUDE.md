...
- `main.rs` — entry, CLI modes (`--rename`, `--workspace-symbol`, `--heatmap`, `--dump-package`, `--parse`, `--clear-cache`, `--version`). `cli_full_startup(root)` = "act like LSP just started". `perl-lsp --heatmap <root> [--csv]` emits per-symbol fan-in/fan-out + unreferenced-symbol (dead-code) candidates over the existing reference graph — a reporting view, not a new tier; honest over-approximation + failure modes in `docs/prompt-heatmap.md`. `perl-lsp --parse <file|-->` prints the tree-sitter parse tree (`--` reads stdin) — use it to check node kinds/fields instead of guessing. `perl-lsp --clear-cache [<root>]` wipes the SQLite module cache — with `<root>` only that project's hashed cache dir, without it the whole `~/.cache/perl-lsp` tree. Prefer this over `rm -rf` (it canonicalizes `<root>` to the same hash the server writes under).
