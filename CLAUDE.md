...
- `main.rs` — entry, CLI modes (`--rename`, `--workspace-symbol`, `--heatmap`, `--dump-package`, `--parse`, `--clear-cache`, `--version`). `cli_full_startup(root)` = "act like LSP just started". `perl-lsp --heatmap <root> [--csv|--html]` emits per-symbol fan-in/fan-out + unreferenced-symbol (dead-code) candidates — fan-in is the CandidateSet's `references()` projection minted at each declaration (never a parallel ref walk), so counts match the references verb by construction; honest over-approximation + failure modes in `docs/prompt-heatmap.md`. JSON is the default; `--csv` is the flat table; `--html` wraps the same report in a self-contained offline viewer (treemap heat + fan-in/fan-out butterfly + dead-code queue) whose template is `src/heatmap.html`, embedded via `include_str!` and printed with the report JSON inlined. `perl-lsp --parse <file|-->` prints the tree-sitter parse tree (`--` reads stdin) — use it to check node kinds/fields instead of guessing. `perl-lsp --clear-cache [<root>]` wipes the SQLite module cache — with `<root>` only that project's hashed cache dir, without it the whole `~/.cache/perl-lsp` tree. Prefer this over `rm -rf` (it canonicalizes `<root>` to the same hash the server writes under).
...
- `resolve.rs` — the resolution CandidateSet (`resolve(cursor)` + the gd/gr/rename/implementations projections), `refs_to`, `RoleMask` (OPEN/WORKSPACE/DEPENDENCY/BUILTIN), import-binding classification. All cross-file queries route here.
