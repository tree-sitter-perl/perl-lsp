...
| references | [references.md](references.md) | 11 | 4 | 0 | 1 |
| hover | [hover.md](hover.md) | 12 | 0 | 4 | 1 |
| type-at | [type-at.md](type-at.md) | 12 | 0 | 2 | 5 |
| rename | [rename.md](rename.md) | 4 | 4 | 1 | 2 |
...
- **ref-expt-mkopt** (references) — split reference graph: def/@EXPORT_OK/call cluster does not cross-link to the three in-body `mkopt(...)` call sites (`:70`, `:105`, `:168`).
- **ref-expt-tag_opts-lexical** (references) — lexical `%$tag_opts` hash-deref spelling at `:173` not emitted as a ref (9/10).
- **ref-uri-escape_char-fq** (references) — non-exported `escape_char`; FQ-only `URI::Escape::escape_char(...)` calls mostly fail to resolve cross-file (contrast exported `uri_unescape`).
- **rename-01-mkopt-def** (rename) — from-def rename drops in-body `mkopt(...)` calls in other sub bodies (`:69/:104/:167`).
- **rename-02-mkopt-callsite** (rename) — from-callsite rename gives the inverse (call-sites only); package resolves None.
- **rename-06-build-exporter** (rename) — in-body `build_exporter($config)` call inside `setup_exporter` (`:602`) dropped.
