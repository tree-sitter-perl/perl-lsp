...
fn decoded_pack_analyses_carry_spellings() {
    ...
    for id in registry.languages() {
        ...
        // BEHAVIOUR (`catch_all_satisfies_contracts`), so it is the one that
        // must not be skipped.
    ...
    assert!(
        LanguageRegistry::spellings("perl").catch_all_satisfies_contracts,
        "perl's pointer is the one that changes behaviour — an unattached \
         decode reports every role obligation an AUTOLOAD answers"
    );
}
