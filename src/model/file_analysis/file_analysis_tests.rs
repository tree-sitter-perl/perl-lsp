...
fn test_fq_method_call_nav_dispatches_from_named_class() {
    use crate::model::file_analysis::{RefKind, RenameKind};
...
fn render_outline(items: &[OutlineSymbol]) -> String {
    fn walk(o: &OutlineSymbol, depth: usize, buf: &mut String) {
        let kind = crate::lsp::symbols::outline_lsp_kind(o);
    ...
    let self_wits: Vec<_> = fa
        ...
        .filter(|w| {
            matches!(&w.attachment,
                crate::model::witnesses::WitnessAttachment::Variable { name, .. } if name == "$self")
        })
        .collect();
    assert!(
        !self_wits.is_empty(),
...
$r->get('/x')->to('Y#z');
    ...
    let expr_wits: Vec<_> = fa
        ...
        .filter(|w| {
            matches!(
                w.attachment,
                crate::model::witnesses::WitnessAttachment::Expression(_)
            )
        })
        .collect();
    assert!(
...
sub boot {
    ...
    assert!(
        ...
        fa.witnesses
            ...
            .filter(|w| matches!(
                &w.attachment,
                crate::model::witnesses::WitnessAttachment::HashKey { .. }
            ))
            .count(),
    );
}
...
"#;
    ...
    let ty = crate::lsp::cursor_context::resolve_expression_type(&fa, invocant, source_bytes, None);
...
fn cross_file_slot_write_types_the_read() {
    ...
    let parent_src = "package Parent;\nsub init {\n    my ($self) = @_;\n    $self->{conn} = My::Conn->new;\n}\n1;\n";
    {
        ...
        let fa = crate::build::builder::build(&tree, parent_src.as_bytes());
        idx.insert_cache(
            "Parent",
    ...
    let fa = crate::build::builder::build(&tree, child_src.as_bytes());
...
fn loader_config_types_register_conf_cross_file() {
    ...
    let app_src2 = "package MyApp::Boot;\nuse Mojolicious::Lite;\nplugin 'CloveApp', { minion => 1, redis => 'r' };\n1;\n";
    {
        ...
        let fa = crate::build::builder::build(&tree, app_src2.as_bytes());
        idx.insert_cache(
            "MyApp::Boot",
            Some(Arc::new(crate::model::file_analysis::CachedModule::new(
