...
//! `crate::index::resolve::refs_to` only, so they're agnostic to the
...
fn match_template_pattern_binds_scores_and_rejects() {
    use crate::model::file_analysis::{match_template_pattern, InferredType};
...
fn substitute_type_params_walks_structure() {
    use crate::model::file_analysis::{substitute_type_params, InferredType};
    let params = vec!["T".to_string()];
