...
fn wsym(sym_id: u32, payload: WitnessPayload) -> Witness {
    Witness {
        attachment: WitnessAttachment::Symbol(crate::model::file_analysis::SymbolId(sym_id)),
...
fn arity_zero_returns_string_default_returns_self() {
    ...
    let reg = ReducerRegistry::with_defaults();
    let att = WitnessAttachment::Symbol(crate::model::file_analysis::SymbolId(sub_id));
...
fn old_plugin_spelling_still_deserializes() {
    ...
    for spelling in [
        ...
        r#"{"MethodOnClass":{"class":"Foo","name":"bar"}}"#,
        r#"{"MethodOnClass":{"package":"Foo","name":"bar"}}"#,
    ] {
