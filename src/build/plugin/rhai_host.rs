...
use crate::model::file_analysis::{HashKeyOwner, InferredType, Span};
...
pub struct RhaiPlugin {
    ...
    load_verbs: Vec<crate::build::plugin::LoadVerb>,
...
impl RhaiPlugin {
    ...
    pub fn from_source(
        ...
        let mut topic_route_dsl: Option<crate::build::plugin::TopicRouteDsl> = None;
        if signatures.iter().any(|n| n == "topic_route_dsl") {
            match engine.call_fn::<Dynamic>(&mut rhai::Scope::new(), &ast, "topic_route_dsl", ()) {
                Ok(d) => match from_dynamic::<crate::build::plugin::TopicRouteDsl>(&d) {
...
impl FrameworkPlugin for RhaiPlugin {
    ...
    fn load_verbs(&self) -> &[crate::build::plugin::LoadVerb] {
        &self.load_verbs
    }
    ...
    fn patterns(&self) -> &[crate::build::plugin::PatternSpec] {
        &self.patterns
    }

    fn on_match(&self, pattern: &str, m: &crate::build::plugin::MatchContext) -> Vec<EmitAction> {
...
mod tests {
    ...
    fn mcap(text: &str, span: Span) -> crate::build::plugin::CaptureData {
        crate::build::plugin::CaptureData {
            text: text.into(),
            span,
            string_value: None,
    ...
    fn one(d: crate::build::plugin::CaptureData) -> crate::build::plugin::CaptureValue {
        crate::build::plugin::CaptureValue::One(Box::new(d))
    }

    #[test]
    fn minimal_plugin_loads_and_dispatches() {
        use crate::build::plugin::MatchContext;
    ...
    fn bundled_mojo_events_loads_and_emits() {
        use crate::build::plugin::{CaptureData, CaptureValue, MatchContext};
    ...
    fn bundled_dbic_resultddl_synthesizes_accessors() {
        ...
        for (func, accessor) in cases {
            ...
            captures.insert(
                "name".to_string(),
                one(crate::build::plugin::CaptureData {
    ...
    fn rhai_overrides_function_is_read_at_compile_time() {
        ...
        assert_eq!(ovs.len(), 1);
        match &ovs[0].target {
            crate::build::plugin::OverrideTarget::Method { class, name } => {
                assert_eq!(class, "Foo");
                assert_eq!(name, "bar");
            }
    ...
    fn catalyst_match(
        ...
        target: &str,
        target_span: Span,
    ) -> crate::build::plugin::MatchContext {
        let mut captures = std::collections::HashMap::new();
