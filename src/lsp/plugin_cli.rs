...
use crate::build::builder::build_with_plugins;
use crate::model::file_analysis::{FileAnalysis, Namespace, Symbol};
use crate::index::module_resolver;
use crate::build::plugin::rhai_host::{make_engine, RhaiPlugin};
use crate::build::plugin::{FrameworkPlugin, PluginRegistry};
...
fn check_entry_declarations(path: &Path, json_mode: bool) {
    ...
    let known_rule_keys = ["attributes", "method_prefix", "methods", "when_isa"];
    ...
    if let Some(entries) = entries {
        for (i, rule) in entries.iter().enumerate() {
            ...
            let Some(obj) = rule.as_object() else {
                warnings.push(format!("rule #{i}: not an object"));
                continue;
            };
            for key in obj.keys() {
                if !known_rule_keys.contains(&key.as_str()) {
                    warnings.push(format!(
                        "rule #{i}: unknown field `{key}` (ignored — a typo here silently weakens the rule)"
                    ));
                }
            }
            let positive = obj.get("attributes").and_then(|v| v.as_array()).is_some_and(|a| !a.is_empty())
                || obj.get("method_prefix").and_then(|v| v.as_str()).is_some()
                || obj.get("methods").and_then(|v| v.as_array()).is_some_and(|a| !a.is_empty());
            if !positive {
                warnings.push(format!(
...
fn diff_emissions(baseline: &FileAnalysis, with_plugin: &FileAnalysis, plugin_id: &str) -> Value {
    ...
    let var_type_key = |w: &crate::model::witnesses::Witness| -> Option<String> {
...
fn ref_to_snapshot(r: &crate::model::file_analysis::Ref) -> Value {
    json!({
        "kind": format!("{:?}", r.kind),
...
fn span_to_json(s: &crate::model::file_analysis::Span) -> Value {
    json!({
...
fn ref_key(r: &crate::model::file_analysis::Ref) -> String {
    format!(
        "{:?}|{}|{}:{}-{}:{}",
...
fn import_key(i: &crate::model::file_analysis::Import) -> String {
    ...
    match RhaiPlugin::from_source(&source, engine) {
        Ok(p) => {
            ...
            for spec in p.patterns() {
                ...
                if let Err(e) = crate::build::builder::pattern_dispatch::verify_pattern_expects(spec) {
