    ...
    loop {
        ...
        for s in fa.symbols() {
            if s.kind == SymKind::Package
                && s.flags.contains(crate::model::file_analysis::SymbolFlags::INLINE)
...
pub(super) fn collect_from_analysis(
    ...
    for &ri in &keyed {
        ...
        // package/class). A member value read matches on the same tail. Every
        // other ref kind matches by exact name.
