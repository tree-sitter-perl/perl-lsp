...
| `prompt-catch-all-surface.md` — what an `AUTOLOAD` / `__call` answers | **Queued after the PHP arc lands and Epic 3 Phase B**, so it narrows the one verdict's `Autoload` cause rather than each lane. Epic 3 Phase A takes the member name for it |
