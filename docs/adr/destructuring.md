...
1. **One binder, language-neutral.** A destructuring slot is a
   ...
   so `[, $b] = …` lands on index 1. A slurpy tail is
   `Extraction::Slurpy(n)`: only `my (@all) = …` (position 0, an array
   target) IS the source and takes the whole-source edge; a tail from a
   later position (`my ($a, @rest) = …`) has no projection step and stays
   untyped rather than claiming the whole source's lattice.
