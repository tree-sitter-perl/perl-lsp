...
| `undefined-property` | the same, for a member READ (`MemberKind::Value`) | error |
...
Where the syntax tells a call from a value read, the extractor says so
at mint time: a read is a `RefKind::FieldAccess` and a call a
`RefKind::MethodCall`, and `MemberKind::of_ref` is the family every
consumer asks (`docs/adr/member-kinds.md`). So
`unresolved-method`/`undefined-property` fire independently even when the
class overloads the name across kinds, and no lane consults a shape tag.
Goto-def and hover read the same declaration: `field_value_type` is the
value rung and `member_value_type` the kind-less ladder, so a value read
of a method-only name is an undeclared property on every verb. A callable
