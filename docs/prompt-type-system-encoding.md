...
2. If we do (1A) phantom types, does plugin-emitted `Custom(idx)`
   parametric semantics (see `adr/return-expr.md` for the
   receiver-relative machinery they'd plug into) need a
   corresponding `Custom<Idx>` axis marker, or do plugins pick from
   the fixed set of axes?
